...
func _setup_armor_buttons() -> void:
	...
	for child in vbox.get_children():
		if child is Button and child.has_node("Item/Quantity"):
			...
			child.mouse_entered.connect(_update_parameter_change.bind(child))
			child.mouse_exited.connect(_on_armor_row_mouse_exited.bind(child))
			_armor_rows.append(child)
	_armor_buy = vbox.get_node_or_null("Buy") as Button
	if _armor_buy != null:
...
func _on_armor_plus_pressed(row: Button) -> void:
	...
	quantity.text = str(int(quantity.text) + 1)
	_update_armor_total()
...
func _on_armor_row_mouse_exited(row: Button) -> void:
	if _selected_armor != null and _selected_armor != row:
		_update_parameter_change(_selected_armor)
	else:
		_reset_parameter_change()
...
func _on_armor_buy_pressed() -> void:
	var total := int(_armor_buy.text)
	if total <= 0 or total > _coins:
		return
	_coins -= total
	if _total_coin_label != null:
